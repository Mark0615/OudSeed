...
IMAGE_NAME="${IMAGE_NAME:-oudseed-ai-report}"
JOB_NAME="${JOB_NAME:-oudseed-ai-report}"
SCHEDULER_JOB_NAME="${SCHEDULER_JOB_NAME:-oudseed-ai-report-monthly}"
...
if [[ -z "${OPENAI_API_KEY:-}" ]]; then
  echo "Missing OPENAI_API_KEY. Set it in ${ENV_FILE} or the environment." >&2
  exit 1
fi
...
gcloud run jobs deploy "${JOB_NAME}" \
  ...
  --set-env-vars="GCP_PROJECT_ID=${PROJECT_ID},BIGQUERY_DATASET=${BIGQUERY_DATASET},AI_REPORT_TYPE=${AI_REPORT_TYPE},AI_REPORT_LIMIT=${AI_REPORT_LIMIT},AI_REPORT_TIMEZONE=${TIME_ZONE},OPENAI_MODEL=${OPENAI_MODEL},OPENAI_REASONING_EFFORT=${OPENAI_REASONING_EFFORT},OPENAI_MAX_OUTPUT_TOKENS=${OPENAI_MAX_OUTPUT_TOKENS}" \
