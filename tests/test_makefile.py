...
def test_makefile_includes_account_report_ops_targets() -> None:
    ...
    assert "onboarding-prototype-real-google:" in makefile
    assert "onboarding-prototype-real-sources:" in makefile
    ...
    assert "ONBOARDING_USE_REAL_META=true" in makefile
    assert "ONBOARDING_USE_REAL_GOOGLE_ADS=true" in makefile
    ...
    assert "ONBOARDING_LIVE_SYNC_PLATFORM=google_ads" in makefile
    assert "SYNC_ENABLED_PLATFORMS=google_ads" in makefile
