"""Generate + email a workspace's AI report for one report schedule.

Shared building blocks for both triggers of the same pipeline:
- the dashboard "send a test report now" button (:mod:`src.web.app`), and
- the automatic scheduled dispatcher (:mod:`src.ai.dispatch_scheduled_reports`).

It resolves a saved schedule, builds the pipeline config from the durable store,
and reuses the deployed report pipeline's discovery + generate + send loop
(:mod:`src.ai.send_account_reports`) so every email matches the scheduled job's
output. Returns a structured outcome instead of raising, so callers can report a
friendly, secret-free status.

This module has no web-framework dependencies despite serving the web app.
"""
...
class SendNowResult:
    """Outcome of a workspace report send.
...
def send_workspace_reports_now(
    *,
    ...
    """Generate and email the workspace's report group(s) for one schedule.

    Returns a structured outcome instead of raising so callers can show a
    friendly status. The heavy generate + send work is delegated to the shared
